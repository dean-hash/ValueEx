...
function hasJSDoc(node) {
  if (
    ...
    ts9__default.default.isCaseClause(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    ts9__default.default.isConstructorDeclaration(node) ||
...
function isSignatureDeclaration(node) {
  return (
    ...
    ts9__default.default.isFunctionDeclaration(node) ||
    ts9__default.default.isMethodDeclaration(node) ||
    ts9__default.default.isConstructorDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    ts9__default.default.isFunctionExpression(node) ||
    ts9__default.default.isArrowFunction(node)
  );
...
function isUnparsedSourceText(node) {
  return (
    ts9__default.default.isUnparsedPrepend(node) || ts9__default.default.isUnparsedTextLike(node)
  );
}
...
function isJsxTagNamePropertyAccess(node) {
  return (
    ts9__default.default.isPropertyAccessExpression(node) && // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts < 5
    ...
    outer: switch (parent.kind) {
      ...
      case ts9__default.default.SyntaxKind.CallExpression:
        ...
        const propertySymbol =
          (_b = (_a = parameterSymbolLinks.type) == null ? void 0 : _a.getProperties()) == null
            ? void 0
            : _b[argumentIndex];
...
function identifierToKeywordKind(node) {
  return 'identifierToKeywordKind' in ts9__default.default
    ? ts9__default.default.identifierToKeywordKind(node)
    : // eslint-disable-next-line deprecation/deprecation
      node.originalKeywordKind;
}
function canHaveDecorators(node) {
  return 'canHaveDecorators' in ts9__default.default
    ? ts9__default.default.canHaveDecorators(node)
    : 'decorators' in node;
}
function getDecorators(node) {
  return 'getDecorators' in ts9__default.default
    ? ts9__default.default.getDecorators(node)
    : node.decorators;
}
