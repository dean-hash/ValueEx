...
export class MerchantOpportunityAnalyzer {
    ...
    private findMatchingDemandPatterns(merchant: MerchantOpportunity): DemandPattern[] {
        return this.demandPatterns.value.filter(pattern => {
            // Category match
            const categoryMatch = merchant.products.categories
                .some(cat => cat.toLowerCase() === pattern.category.toLowerCase());
    ...
    private calculateOpportunityScore(
        ...
        const scores = patterns.map(pattern => {
            ...
            score += this.calculatePriceAlignment(
                merchant.products.priceRange,
                pattern.pricePoint
            ) * 0.2;
    ...
    private calculatePriceAlignment(
        ...
        const overlap = Math.min(merchantRange.max, demandPoint.max) -
                       Math.max(merchantRange.min, demandPoint.min);
