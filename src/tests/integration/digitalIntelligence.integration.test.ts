...
describe('Digital Intelligence Integration Tests', () => {
  ...
  it('should provide vertical-specific recommendations', async () => {
    ...
    expect(
      analysis.signals.some(
        (s) =>
          s.metadata.vertical?.characteristics.seasonality ===
          MARKET_VERTICALS.fashion.characteristics.seasonality
      )
    ).toBe(true);
  });
  ...
  it('should consider vertical characteristics in urgency analysis', async () => {
    ...
    const consideredUrgency =
      consideredProduct.signals.find((s) => s.type === 'urgency')?.strength || 0;
