...
describe('Demand Validation Integration Tests', () => {
  ...
  const realWorldScenarios = [
    {
      ...
      signal: {
        ...
        content:
          'Looking for a high-performance laptop for gaming and video editing, budget around $2000',
    ...
    },
    {
      ...
      signal: {
        ...
        content:
          'Looking for compact home gym equipment for a small apartment, need something versatile',
  ...
  describe('Real-world Scenario Validation', () => {
    test.each(realWorldScenarios)(
      'should validate $name demand pattern',
      async ({ signal }) => {
        ...
        expect(enhancedSignal.context.keywords.length).toBeGreaterThanOrEqual(
          signal.context.keywords.length
        );
    ...
    it('should identify emerging trends across multiple signals', async () => {
      ...
      const patterns = await Promise.all(
        signals.map((signal) => enhancer.enhanceDemandContext(signal))
      );
    ...
    it('should maintain context consistency across enhancements', async () => {
      ...
      const confidenceDiff = Math.abs(
        firstEnhancement.context.sentiment - secondEnhancement.context.sentiment
      );
