...
describe('LocalIntelligenceProvider', () => {
  ...
  const validSignal: DemandSignal = {
    id: 'test-signal-1',
  ...
  const invalidSignal: DemandSignal = {
    id: 'invalid-signal',
    title: '',
    content: '',
    url: '',
  ...
  const createTestSignal = (i: number): DemandSignal => ({
    id: `test-signal-${i}`,
    title: `Test Signal ${i}`,
  ...
  it('should handle batch processing efficiently', async () => {
    const mockSignals: DemandSignal[] = Array(5)
      .fill(null)
      .map((_, i) => createTestSignal(i));
