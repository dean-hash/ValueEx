...
describe('DemandInference', () => {
  ...
  describe('Behavior Analysis', () => {
    ...
    test('should analyze viewing patterns', async () => {
      ...
      const viewSignal = signals.find((s) =>
        s.context.keywords.some((k) => mockBehavior.viewedItems.some((item) => item.includes(k)))
      );
    ...
    test('should analyze interactions', async () => {
      ...
      const interactionSignal = signals.find(
        (s) => s.context.urgency > 0 || s.context.sentiment !== 0
      );
  ...
  describe('Signal Consolidation', () => {
    const mockSignals: DemandSignal[] = [
      {
        id: 'test-signal-1',
        title: 'Test Signal',
