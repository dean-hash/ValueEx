...
export class PortfolioAnalyzer {
  ...
  private field: UnifiedIntelligenceField;
  ...
  async analyzeDomainMetrics(domain: string): Promise<DomainMetrics> {
  ...
  private determineStrategy(analysis: any) {
    const strategies = {
  ...
  private determineNiche(domain: string): string {
  ...
  private calculateDomainValue(analysis: any): number {
    ...
    if (domainLength < 6) multiplier *= 1.5;
    if (domainLength > 15) multiplier *= 0.7;
  ...
  async getAffiliateRecommendations(domain: string): Promise<any[]> {
    const analysis = await this.analyzer.analyzeDomain(domain);
