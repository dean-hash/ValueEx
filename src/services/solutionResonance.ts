...
export class SolutionResonance {
    ...
    private async calculatePropertyResonance(
        solution: Solution,
        context: Context
    ): Promise<number> {
        let score = 0;
        ...
        if (prefs.pricePreference) {
            score += this.alignmentScore(
                solution.properties.price,
                prefs.pricePreference
            );
        }
        ...
        if (prefs.timeConstraint) {
            score += this.alignmentScore(
                solution.properties.deliveryTime,
                prefs.timeConstraint
            );
        }
        ...
        if (prefs.qualityEmphasis) {
            score += this.alignmentScore(
                solution.properties.quality,
                prefs.qualityEmphasis
            );
        }
    ...
    private async calculateHistoricalResonance(
        ...
        return similarSolutions.reduce((sum, prev) => sum + prev.satisfaction, 0) / 
               similarSolutions.length;
    }

    private async calculateSituationalFit(
        solution: Solution,
        context: Context
    ): Promise<number> {
        // This could include:
