...
export class TeamsClient {
    ...
    async createChannel(teamId: string, channelName: string, description: string): Promise<Channel> {
        try {
            const channel = await this.graphClient.api(`/teams/${teamId}/channels`)
                .post({
    ...
    async sendMessage(channelId: string, content: string): Promise<Message> {
        try {
            const message = await this.graphClient.api(`/teams/${channelId}/messages`)
                .post({
                    body: {
                        content: content,
                        contentType: 'html'
    ...
    async joinTeamsMeeting(meetingId: string): Promise<{ joinUrl: string }> {
        try {
            const joinInfo = await this.graphClient.api(`/me/onlineMeetings/${meetingId}/joinWebUrl`)
                .get();
