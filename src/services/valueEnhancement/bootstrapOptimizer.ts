...
export class BootstrapOptimizer {
  private static instance: BootstrapOptimizer;
  ...
  async findValueOpportunity(signal: DemandSignal): Promise<{
    score: ValueScore;
    actions: string[];
    resources: string[];
  }> {
    try {
      ...
      logger.error('Error finding value opportunity:', error);
      throw error;
    }
  }

  private async scoreValue(signal: DemandSignal): Promise<ValueScore> {
  ...
  private scoreAuthenticity(signal: DemandSignal): number {
    ...
    let score = 0;
    if (hasContext) score += 0.4;
    if (hasKeywords) score += 0.3;
    if (highConfidence) score += 0.3;

    return score;
  }

  private scoreUtility(signal: DemandSignal): number {
    ...
    const practicalCount = keywords.filter((word) =>
      practicalTerms.some((term) => word.toLowerCase().includes(term))
    ).length;

    return Math.min(practicalCount / 3, 1);
  }

  private scoreSustainability(signal: DemandSignal): number {
    ...
    if (hasDepth) score += 0.4;
    if (hasConfidence) score += 0.3;

    return score;
  }

  private generateActions(score: ValueScore): string[] {
  ...
  private async findFreeResources(signal: DemandSignal): Promise<string[]> {
  ...
  async validateValue(signal: DemandSignal): Promise<{
    isViable: boolean;
    nextSteps: string[];
    risks: string[];
  }> {
