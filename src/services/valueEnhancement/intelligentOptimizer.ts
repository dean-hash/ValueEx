...
interface ResourceMetrics {
  attention: number; // Available attention units
  knowledge: number; // Accumulated knowledge
  community: number; // Community engagement level
  impact: number; // Measured impact of actions
}
...
export class IntelligentOptimizer {
  private static instance: IntelligentOptimizer;
  private patterns: Map<string, OpportunityInsight>;
  ...
  private async learnFromSignal(signal: DemandSignal): Promise<void> {
    ...
    for (const pattern of patterns) {
      ...
      if (signal.insights.keywords) {
        existing.applications = Array.from(
          new Set([...existing.applications, ...signal.insights.keywords])
        );
      }
  ...
  private extractPatterns(signal: DemandSignal): string[] {
    const patterns: string[] = [];

    // Extract timing patterns
    if (signal.insights.urgency > 0.7) patterns.push('high_urgency');
    if (signal.insights.confidence > 0.8) patterns.push('high_confidence');

    // Extract context patterns
    if (signal.insights.context) {
      if (signal.insights.context.includes('problem')) patterns.push('problem_solving');
      if (signal.insights.context.includes('improve')) patterns.push('improvement');
      if (signal.insights.context.includes('need')) patterns.push('direct_need');
    }

    // Extract keyword patterns
    if (signal.insights.keywords) {
      if (signal.insights.keywords.some((k) => k.includes('help'))) patterns.push('help_request');
      if (signal.insights.keywords.some((k) => k.includes('build'))) patterns.push('creation');
      if (signal.insights.keywords.some((k) => k.includes('learn'))) patterns.push('learning');
    }

    return patterns;
  }

  private async generateAttentionBid(signal: DemandSignal): Promise<AttentionBid> {
    ...
    // Calculate priority
    const priority =
      signal.insights.confidence * 0.3 + signal.insights.urgency * 0.3 + patternEffectiveness * 0.4;

    // Calculate attention allocation
    const allocation = Math.min(
      this.resourceMetrics.attention * priority,
      this.resourceMetrics.attention * 0.8 // Never allocate more than 80% of attention
    );
  ...
  private generateStrategy(patterns: string[], signal: DemandSignal): string[] {
    ...
    for (const pattern of patterns) {
      ...
      if (insight && insight.effectiveness > 0.7) {
        strategy.push(
          `Apply ${pattern} approach - ${insight.effectiveness.toFixed(2)} effectiveness`
        );
      }
    }
  ...
  private generateInsights(signal: DemandSignal): string[] {
    ...
    for (const pattern of patterns) {
      ...
      if (insight && insight.applications.length > 0) {
        insights.push(
          `Pattern ${pattern} has been effective in: ${insight.applications.join(', ')}`
        );
      }
    }
  ...
  private getRelevantLearnings(signal: DemandSignal): OpportunityInsight[] {
    return Array.from(this.patterns.values())
      .filter((insight) => insight.effectiveness > 0.6)
      .sort((a, b) => b.effectiveness - a.effectiveness);
  }

  private updateResourceMetrics(signal: DemandSignal, bid: AttentionBid): void {
    // Update attention
    this.resourceMetrics.attention = Math.max(0, this.resourceMetrics.attention - bid.allocation);

    // Update knowledge
    this.resourceMetrics.knowledge = Math.min(
      1,
      this.resourceMetrics.knowledge + bid.allocation * 0.1
    );

    // Update community if signal shows engagement
    if (
      signal.insights.keywords?.some(
        (k) => k.includes('community') || k.includes('help') || k.includes('together')
      )
    ) {
    ...
    this.resourceMetrics.impact = Math.min(
      1,
      this.resourceMetrics.impact + signal.insights.confidence * 0.1
    );
  }
