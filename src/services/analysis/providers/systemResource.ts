...
export class SystemResourceProvider extends EventEmitter implements IntelligenceProvider {
  ...
  private async getSystemMetrics(): Promise<SystemMetrics> {
    return new Promise((resolve, reject) => {
      exec('powershell -Command "Get-Process | Sort-Object -Property WS -Descending | Select-Object -First 10 ProcessName, @{Name=\'MemoryMB\';Expression={[math]::Round($_.WS / 1MB, 2)}}, CPU"', 
        (error, stdout) => {
          if (error) {
            reject(error);
            return;
  ...
  async optimizeIDE(): Promise<void> {
    // Clean up temp files
    exec('powershell -Command "Remove-Item $env:TEMP\\* -Recurse -Force -ErrorAction SilentlyContinue"');
    
    // Optimize VSCode/Windsurf processes
    const metrics = await this.getSystemMetrics();
    const ideProcesses = metrics.topProcesses
      .filter(p => p.name.toLowerCase().includes('windsurf'));
