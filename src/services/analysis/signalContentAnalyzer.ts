...
export class SignalContentAnalyzer {
  private static instance: SignalContentAnalyzer;
  ...
  async analyzeContent(signal: DemandSignal): Promise<ContentMetrics> {
    try {
      ...
      logger.error('Error analyzing signal content:', error);
      throw error;
    }
  }

  private async gatherContent(signal: DemandSignal): Promise<string> {
  ...
  private async analyzeUniquePoints(content: string): Promise<number> {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Analyze the following content and identify unique, substantive points that add real value. Count only points that contribute meaningful insights or practical utility.',
        },
        {
          role: 'user',
          content,
  ...
  private async assessDetailDepth(content: string): Promise<number> {
    const response = await this.model.chat.completions.create({
      model: 'gpt-4',
      messages: [
        {
          role: 'system',
          content:
            'Analyze the depth and specificity of the following content. Consider:\n1. Technical accuracy\n2. Contextual understanding\n3. Implementation details\n4. Real-world applicability\nScore from 0 to 1.',
        },
        {
          role: 'user',
          content,
  ...
  private async assessPracticalValue(content: string): Promise<number> {
    const response = await this.model.chat.completions.create({
      model: 'gpt-4',
      messages: [
        {
          role: 'system',
          content:
            'Evaluate the practical value of this content. Consider:\n1. Actionability\n2. Problem-solving utility\n3. Real-world impact\n4. Cost-benefit ratio\nScore from 0 to 1.',
        },
        {
          role: 'user',
          content,
  ...
  private async evaluateSourceCredibility(source: string): Promise<number> {
    // This would connect to a reputation database or analysis system
    // For MVP, returning a default score
    return 0.8;
  }

  private async analyzeCommunityEngagement(signal: DemandSignal): Promise<{
    quality: number;
    volume: number;
    sustainability: number;
  }> {
  ...
  private async fetchExternalContent(signal: DemandSignal): Promise<string> {
    // This would fetch content from external sources
    // For MVP, returning empty string
    return '';
  }

  private extractPointCount(analysis: string): number {
