...
async function testMerchants() {
  ...
  try {
    ...
    merchants.forEach((merchant: any) => {
      ...
      console.log(
        `Commission Details: ${merchant.commissionDetails || merchant.commissionRate || 'Not specified'}`
      );
      console.log(`Region: ${merchant.region || merchant.primaryRegion || 'Not specified'}`);
      console.log(
        `Description: ${merchant.description || merchant.strapLine || 'No description provided'}`
      );
      console.log('-------------------\n');
    });
  } catch (error) {
