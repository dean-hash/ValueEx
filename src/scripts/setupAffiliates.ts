...
async function getAutoApprovePrograms(): Promise<AffiliateSignup[]> {
...
async function main() {
  try {
    ...
    console.log('\nNext Steps:');
    console.log(
      '1. Start with Amazon Associates - largest marketplace with AI/ML books and courses'
    );
    console.log('2. Set up ClickBank for digital AI products');
    console.log('3. Use ShareASale for AI tool vendors');
  } catch (error) {
    logger.error('Error checking affiliate programs', error);
  }
}
