import { IntelligenceField } from '../core/unified/intelligenceField';
...
async function detectAndManifestValue() {
  ...
  const field = new IntelligenceField();
  ...
  merchants.forEach((merchant1) => {
    merchants.forEach((merchant2) => {
      if (merchant1.merchantId !== merchant2.merchantId) {
        const sharedCategories = merchant1.categories.filter((cat) =>
  ...
  field.observeResonancePatterns().subscribe(async (pattern) => {
    ...
    const highestResonance = pattern.affectedNodes.sort(
      (a, b) => b.resonanceStrength - a.resonanceStrength
    )[0];
    ...
    if (highestResonance) {
      ...
      if (merchant) {
        ...
        console.log(
          `Resonance Strength: ${(highestResonance.resonanceStrength * 100).toFixed(1)}%`
        );
