...
export class MatchingEngine {
  ...
  public async findMatches(request: MatchRequest): Promise<Match[]> {
    try {
      ...
      matches.filter((match) => match.matchScore > 0.8).forEach((match) => this.notifyMatch(match));
  ...
  private mergeAndSortResults(matches: Match[]): Match[] {
    return matches.sort((a, b) => b.matchScore - a.matchScore).slice(0, 100); // Limit to top 100 matches
  }
  ...
  private async calculateMatchScore(product: MVPProduct, signal: MVDDemand): Promise<number> {
    ...
    if (
      signal.priceRange &&
      product.price >= signal.priceRange.min &&
      product.price <= signal.priceRange.max
    ) {
      score += 0.3;
    }
...
function calculateMatch(product: Product, demand: DemandSignal): Match {
  ...
  const matchScore =
    ((categoryMatch ? 0.4 : 0) + (priceMatch ? 0.3 : 0) + descriptionSimilarity * 0.3) *
    (demand.confidence || 1);
