...
import { MarketTrends, PriceRange, UserPreferences } from '../../../types/market';
...
describe('MatchingEngine', () => {
  ...
  describe('findMatches', () => {
    ...
    it('should handle errors gracefully', async () => {
      ...
      jest.spyOn(matchingEngine as any, 'handleWorkerError').mockImplementation(() => {
        throw error;
      });

      await expect(matchingEngine.findMatches(testRequest)).rejects.toThrow('Worker error');
  ...
  describe('processSignal', () => {
    it('should process demand signals correctly', async () => {
      const signal: DemandSignal = {
        market: 'test-market',
        category: 'test-category',
        keywords: ['test', 'keywords'],
        intent: 'purchase',
        priceRange: { min: 0, max: 100 } as PriceRange,
      ...
      const result = await (matchingEngine as any).processSignal(signal);
