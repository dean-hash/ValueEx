...
class DependencyGraph {
     ...
     */
    getAllDependencies(node, visited = new Set()) {
        const deps = new Set();
     ...
     */
    hasCycle(node, visited = new Set(), stack = new Set()) {
        if (!node) {
     ...
     */
    getAllDependents(node, visited = new Set()) {
     ...
     */
    calculateAverageDependencyDepth() {
        let totalDepth = 0;
        let nodeCount = 0;
        for (const node of this.graph.keys()) {
