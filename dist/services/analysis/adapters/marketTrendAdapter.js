...
exports.MarketTrendAdapter = void 0;
const events_1 = require("events");
class MarketTrendAdapter extends events_1.EventEmitter {
    constructor() {
        super();
    ...
    async addSignal(category, region, signal) {
        ...
        this.activeSignals.set(key, signals.filter((s) => new Date(s.timestamp) > cutoff));
        // Process immediately if we have enough signals
        if (signals.length >= 10) {
            await this.processSignals(category, region);
        }
    }
    async processSignals(category, region) {
        const key = `${category}_${region}`;
        const signals = this.activeSignals.get(key) || [];
        if (signals.length === 0)
            return;
        ...
        // Group by source
        const sourceGroups = signals.reduce((groups, signal) => {
    ...
    calculateSourceStrength(signals) {
...
exports.MarketTrendAdapter = MarketTrendAdapter;
//# sourceMappingURL=marketTrendAdapter.js.map