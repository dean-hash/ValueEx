...
const metricsCollector_1 = require("../services/metrics/metricsCollector");
const logger_1 = require("../utils/logger");
async function analyzeDomainPortfolio() {
    const connector = godaddyConnector_1.GoDaddyConnector.getInstance();
    const revenueTracker = new revenueTracker_1.RevenueTracker();
    const metrics = new metricsCollector_1.MetricsCollector();
    try {
        ...
        const analyses = await Promise.all(activeDomains.map(async (domain) => {
        ...
        analyses
            ...
            a.matchingPotential.revenueStreams.forEach((stream) => {
                console.log(`- ${stream.type}: $${stream.estimatedValue} (${Math.round(stream.probability * 100)}% probability)`);
            });
        });
        return analyses;
    }
    catch (error) {
        logger_1.logger.error('Error analyzing domain portfolio:', error);
        throw error;
    }
}
analyzeDomainPortfolio();
//# sourceMappingURL=analyzeDomainPortfolio.js.map