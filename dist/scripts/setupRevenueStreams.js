    ...
    for (const domain of premiumDomains) {
        if (!excludedDomains.includes(domain.domain)) {
            try {
                ...
                logger_1.logger.error(`Failed to list ${domain.domain}`, error);
            }
        }
    }
}
async function setupAffiliateLinks() {
    ...
    console.log('==============================');
    const affiliateManager = affiliateManager_1.AffiliateManager.getInstance();
    ...
    for (const program of programs) {
        try {
            ...
            logger_1.logger.error(`Failed to set up ${program.name} affiliate`, error);
        }
    }
...
async function main() {
    try {
        ...
        logger_1.logger.error('Error in revenue setup', error);
    }
}
main();
//# sourceMappingURL=setupRevenueStreams.js.map