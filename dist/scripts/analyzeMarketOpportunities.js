...
async function getDomainMarketValue(domain) {
    try {
        // Use GoDaddy's aftermarket API to get real market data
        const response = await axios_1.default.get(`https://api.godaddy.com/v1/aftermarket/valuation/${domain}`, {
            headers: {
        ...
        logger_1.logger.error('Error getting domain market value', error);
...
async function analyzeMarketOpportunities() {
    ...
    opportunities.sort((a, b) => {
        const getAnnualValue = (opp) => {
            if (opp.recommendedPath === 'Quick Sale')
                return opp.quickSaleValue;
...
//# sourceMappingURL=analyzeMarketOpportunities.js.map