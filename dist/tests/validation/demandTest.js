...
async function testDemandValidation() {
    const validator = demandValidator_1.DemandValidator.getInstance();
    const results = [];
    logger_1.logger.info('Starting comprehensive demand validation test');
    for (const query of REAL_WORLD_QUERIES) {
        try {
            ...
            results.push(result);
            logger_1.logger.info('Validation result:', {
                query,
            ...
            logger_1.logger.error('Error testing query:', query, error);
        }
    }
    // Analyze results
...
function calculateUrgency(query, validation) {
    ...
    const hasProblemKeywords = problemKeywords.some((keyword) => query.toLowerCase().includes(keyword));
    return (hasUrgentKeywords ? 0.6 : 0) + (hasProblemKeywords ? 0.4 : 0);
}
function calculatePriceAwareness(query, validation) {
    // Price mentions
    const hasPricePoint = query.includes('$') ||
...
function calculateSpecificity(query, validation) {
    // Product details
    const hasModelNumbers = /[A-Za-z0-9]+-[A-Za-z0-9]+/.test(query);
    const hasSpecs = query.includes('"') ||
...
function calculateSolvability(query, validation) {
    ...
    const hasReasonableExpectations = !query.toLowerCase().includes('best') || !query.toLowerCase().includes('perfect');
    return (hasKnownCategory ? 0.6 : 0) + (hasReasonableExpectations ? 0.4 : 0);
}
function analyzeResults(results) {
    // Business-focused analysis
    const actionableLeads = results.filter((r) => r.isReal &&
        r.businessMetrics.solvability > 0.7 &&
        (r.businessMetrics.urgency > 0.6 || r.businessMetrics.specificity > 0.7));
    const highValueLeads = actionableLeads.filter((r) => r.businessMetrics.priceAwareness < 0.5 && r.businessMetrics.specificity > 0.8);
    logger_1.logger.info('Business Analysis:', {
        ...
        topActionableQueries: actionableLeads
            .sort((a, b) => b.businessMetrics.urgency +
...
//# sourceMappingURL=demandTest.js.map