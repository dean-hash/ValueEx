...
exports.ResonanceGestures = void 0;
const THREE = __importStar(require("three"));
const rxjs_1 = require("rxjs");
const operators_1 = require("rxjs/operators");
const valueOpportunityHandler_1 = require("../services/opportunities/valueOpportunityHandler");
class ResonanceGestures {
    constructor(canvas) {
        this.gestureFields = new Map();
        this.intentionField = new Set();
        this.resonanceStrength = 0;
        this.valueOpportunities = new Map();
        this.canvas = canvas;
        this.opportunityHandler = valueOpportunityHandler_1.ValueOpportunityHandler.getInstance();
    ...
    initializeGestureField() {
        ...
        (0, rxjs_1.fromEvent)(this.canvas, 'mousemove')
    ...
    updateGestureField(position) {
        this.gestureFields.forEach((field, key) => {
            ...
            field.direction.set(position.x + (Math.random() - 0.5) * quantumEffect, position.y + (Math.random() - 0.5) * quantumEffect);
    ...
    createEmergentPattern(fieldKey, field) {
    ...
    createQuantumWave(group, field) {
        const points = [];
        const uncertainty = field.quantumState?.uncertainty || 0;
        for (let i = 0; i < 50; i++) {
            const t = i / 50;
            const quantum = Math.sin(t * Math.PI * 10) * uncertainty;
            points.push(new THREE.Vector3(t * 4 - 2, Math.sin(t * Math.PI * 4) * (1 + quantum), Math.cos(t * Math.PI * 3) * quantum));
        }
    ...
    createEntangledSpiral(group, field) {
        const points = [];
        const uncertainty = field.quantumState?.uncertainty || 0;
        ...
        for (let i = 0; i < 200; i++) {
            ...
            points.push(new THREE.Vector3(Math.cos(angle) * radius * (1 + quantum), Math.sin(angle) * radius * (1 + quantum), t * 2 + Math.cos(angle * 3) * quantum));
            // Create entangled echoes
            if (entanglementCount > 0) {
                const echoAngle = angle + Math.PI / entanglementCount;
                points.push(new THREE.Vector3(Math.cos(echoAngle) * radius * (1 - quantum), Math.sin(echoAngle) * radius * (1 - quantum), t * 2 - Math.cos(echoAngle * 3) * quantum));
            }
        }
        const curve = new THREE.CatmullRomCurve3(points);
    ...
    createSuperpositionPulse(group, field) {
        const superposition = field.quantumState?.superposition || 0;
        const uncertainty = field.quantumState?.uncertainty || 0;
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const position = new THREE.Vector3((Math.random() - 0.5) * uncertainty, (Math.random() - 0.5) * uncertainty, (Math.random() - 0.5) * uncertainty);
    ...
    createResonanceFlow(group, field) {
        const points = [];
        const uncertainty = field.quantumState?.uncertainty || 0;
        ...
        for (let i = 0; i < 100; i++) {
            ...
            points.push(new THREE.Vector3(t * 4 - 2 + flowOffset, Math.sin(t * Math.PI * 2) * flowStrength * (1 + quantum), Math.cos(t * Math.PI * 3) * flowStrength * (1 + quantum)));
        }
        const curve = new THREE.CatmullRomCurve3(points);
    ...
    emitValueSignal(signal) {
        const opportunity = {
            ...
            resonance: signal.resonancePattern.harmonics.reduce((sum, h) => sum + h, 0) /
    ...
    createWaveGesture(group, gesture) {
    ...
    createSpiralGesture(group, gesture) {
        const points = [];
        for (let i = 0; i < 100; i++) {
            const t = i / 100;
    ...
    createPulseGesture(group, gesture) {
    ...
    createFlowGesture(group, gesture) {
        const points = [];
        for (let i = 0; i < 50; i++) {
            const t = i / 50;
            points.push(new THREE.Vector3(Math.sin(t * Math.PI * 4) * gesture.amplitude, t * gesture.amplitude * 2, Math.cos(t * Math.PI * 4) * gesture.amplitude));
        }
        const curve = new THREE.CatmullRomCurve3(points);
    ...
    calculatePotentialPaths(position, radius) {
        const paths = [];
        const numPaths = 3 + Math.floor(Math.random() * 3);
        for (let i = 0; i < numPaths; i++) {
            const angle = (i / numPaths) * Math.PI * 2;
            const distance = radius * (1 + Math.random());
            paths.push(new THREE.Vector3(position.x + Math.cos(angle) * distance, position.y + Math.sin(angle) * distance, position.z + (Math.random() - 0.5) * distance));
        }
        return paths;
    }
    animatePulse(pulse, probability) {
    ...
    emitValueOpportunity(id, opportunity) {
    ...
    visualizeOpportunity(opportunity) {
        // Visualize opportunity
    }
}
exports.ResonanceGestures = ResonanceGestures;
//# sourceMappingURL=resonanceGestures.js.map