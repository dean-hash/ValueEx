...
class MVPMetrics {
    constructor() {
        // Ethical Metrics
        this.ethicalMetrics = new rxjs_1.BehaviorSubject({
            transparencyScore: 1,
            fairnessIndex: 1,
            userPrivacyScore: 1,
        ...
        // Revenue Metrics
        this.revenueMetrics = new rxjs_1.BehaviorSubject({
            totalRevenue: 0,
            commissionsEarned: 0,
            transactionVolume: 0,
            averageOrderValue: 0,
        ...
        // Efficiency Metrics
        this.efficiencyMetrics = new rxjs_1.BehaviorSubject({
            averageMatchTime: 0,
            timeToValue: 0,
            systemLatency: 0,
        ...
        // Composite Score
        this.mvpScore = new rxjs_1.BehaviorSubject({
            ethicalScore: 1,
            revenueScore: 0,
            efficiencyScore: 0,
            overallScore: 0,
        });
        this.logger = new logger_1.Logger('MVPMetrics');
    ...
    initializeMetricsMonitoring() {
        ...
        (0, rxjs_1.combineLatest)([this.ethicalMetrics, this.revenueMetrics, this.efficiencyMetrics])
            .pipe((0, operators_1.map)(([ethical, revenue, efficiency]) => this.calculateMVPScore(ethical, revenue, efficiency)))
            .subscribe((score) => {
     ...
     */
    updateRevenueMetrics(metrics) {
        const current = this.revenueMetrics.getValue();
        const updated = { ...current, ...metrics };
        // Calculate growth rate
        if (metrics.totalRevenue) {
            updated.revenueGrowthRate = this.calculateGrowthRate(current.totalRevenue, metrics.totalRevenue);
        }
     ...
     */
    calculateMVPScore(ethical, revenue, efficiency) {
        ...
        const overallScore = ethicalScore *
    ...
    calculateEthicalScore(metrics) {
        ...
        let score = Object.entries(weights).reduce((score, [key, weight]) => score + metrics[key] * weight, 0);
...
exports.MVPMetrics = MVPMetrics;
//# sourceMappingURL=mvpMetrics.js.map