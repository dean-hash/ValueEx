...
class DashboardManager {
    ...
    initializeCharts() {
        ...
        for (const [key, config] of Object.entries(chartConfigs)) {
            this.charts[key] = new Chart(document.getElementById(`${key}Chart`).getContext('2d'), this.createChartConfig(config.label, config.color));
        }
    }
    ...
    async updateMonetizationMetrics() {
        try {
            ...
            const analysisResults = await Promise.all(domains.map(domain => this.analyzer.analyzeDomain(domain)));
    ...
    async initializeDomainPortfolioVisualization() {
        ...
        const domainValues = await Promise.all(this.activeDomains.map(async (domain) => {
